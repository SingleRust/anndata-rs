        ...
        Self: Sized,
    {
        ...
        match data_type {
            DataType::CsrMatrix(_, _) | DataType::CscMatrix(_, _) => {
                ...
                let major_bounds = SelectInfoElemBounds::new(&selection[major_axis], shape[major_axis]);
                ...
                let res = if runs.is_empty() {
                    ...
                    let run_results: Vec<anyhow::Result<(Vec<u64>, Vec<T>, Vec<N>)>> = runs
                        ...
                        .map(|&(start, end)| {
                            let indptr = indptr_ds
                                .read_array_slice::<u64, _, Ix1>(&[SelectInfoElem::from(start..end + 1)])?
                                .into_raw_vec_and_offset()
                                .0;
                    ...
                    let total_nnz: usize = run_parts.iter().map(|(_, indices, _)| indices.len()).sum();
                    ...
                    if major_axis == 0 {
                        ...
                        CsMatI::try_new_csc((minor_len, major_len), new_indptr, new_indices, new_data)
                            .map_err(|(_, _, _, e)| anyhow::anyhow!("Cannot read csc matrix {}", e))?
                    }
                };
